import http from "k6/http";
import { Options } from "k6/options";
import { describe, expect } from "https://jslib.k6.io/k6chaijs/4.3.4.2/index.js";
import { getConfiguration, getAuthorizationHeader, Size } from "./utils";

const configuration = getConfiguration();

const apiEndpoint = configuration.host + "/api/v1";

interface Data {
    authToken: string;
    messageId: string;
}

function size(): Size {
    switch (configuration.size) {
        case "S":
            return { vus: 1, iterations: 10 };
        case "M":
            return { vus: 10, iterations: 50 };
        case "L":
            return { vus: 50, iterations: 100 };
        default:
            throw new Error("Invalid 'Size' value: " + configuration.size);
    }
}

export const options: Options = {
    vus: size().vus,
    thresholds: {
        http_req_duration: ["p(90)<50", "p(98)<100"]
    },
    iterations: size().iterations
};

export function setup(): Data {
    const authToken = getAuthorizationHeader(configuration);

    const body = {
        recipients: [
            {
                address: "id1CsU6VwBMgBA4TLc61nj895ZD1mPjbddNW",
                encryptedKey: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
            }
        ],
        body: "AAAA",
        attachments: []
    };
    const messageId = http
        .post(`${apiEndpoint}/Messages`, JSON.stringify(body), {
            headers: {
                "Content-Type": "application/json",
                Authorization: authToken
            }
        })
        .json("result.id")!
        .toString();

    return {
        authToken: authToken,
        messageId: messageId
    };
}

export default function (data: Data): void {
    describe("Get a Message by Id:", () => {
        const response = http.get(`${apiEndpoint}/Messages/${data.messageId}`, {
            headers: {
                Authorization: data.authToken
            }
        });
        expect(response.status, "response status").to.equal(200);
    });
}
