...
Identity changes feature flags
    ...
    Scenario: Peer changes feature flags and an external event is sent due to a relationship template allocation
    ...
    Scenario: Peer changes feature flags and an external event is sent based due to an acive relationship
        Given Identities i1 and i2
        And a Relationship Template t created by i1
    ...
    Scenario: Peer changes feature flags and an external event is sent due to a pending relationship
        Given Identities i1 and i2
        And a pending Relationship r1 between i1 and i2
        When i2 sends a PATCH request to the /Identities/Self/FeatureFlags endpoint with feature1 enabled and feature2 disabled
        And 2 second(s) have passed
        Then i1 receives an ExternalEvent e of type PeerFeatureFlagsChanged which contains the address of i2
