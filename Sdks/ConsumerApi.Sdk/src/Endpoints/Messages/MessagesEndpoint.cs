...
public class MessagesEndpoint(EndpointClient client) : ConsumerApiEndpoint(client)
{
    ...
    {
        return await _client.Get<ListMessagesResponse>($"api/{API_VERSION}/Messages", null, pagination);
    }

    public async Task<ApiResponse<ListMessagesResponse>> ListMessages(IEnumerable<string> ids, PaginationFilter? pagination = null)
    {
        return await _client.Request<ListMessagesResponse>(HttpMethod.Get, $"api/{API_VERSION}/Messages")
            .Authenticate()
    ...
    public async Task<ApiResponse<Message>> GetMessage(string id, bool noBody = false)
    {
        return await _client.Request<Message>(HttpMethod.Get, $"api/{API_VERSION}/Messages/{id}")
            .Authenticate()
            .AddQueryParameter("noBody", noBody)
            .Execute();
    }

    public async Task<ApiResponse<SendMessageResponse>> SendMessage(SendMessageRequest request)
    {
        return await _client.Post<SendMessageResponse>($"api/{API_VERSION}/Messages", request);
    }
}
