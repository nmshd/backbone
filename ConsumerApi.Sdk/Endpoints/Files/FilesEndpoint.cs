...
public class FilesEndpoint(EndpointClient client) : Endpoint(client)
{
    ...
    public async Task<ApiResponse<FileMetadata>> GetFileMetadata(string id) => await _client.Get<FileMetadata>($"Files/{id}/metadata");

    public async Task<RawApiResponse> DownloadFile(string id) => await _client
        .Request<RawApiResponse>(HttpMethod.Get, $"Files/{id}")
        .Authenticate()
        .ExecuteRaw();
}
