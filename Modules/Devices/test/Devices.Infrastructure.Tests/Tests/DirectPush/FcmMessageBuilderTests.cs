...
public class FcmMessageBuilderTests
{
    ...
    public void Built_message_has_all_properties_set()
    {
        ...
        var message = new FcmMessageBuilder()
            ...
            .AddContent(new NotificationContent(IdentityAddress.Parse("did:web:prod.enmesh.eu:dids:id1KJnD8ipfckRQ1ivAhNVLtypmcVM5vPX4j"), DevicePushIdentifier.New(), new { SomeProperty = "someValue" }))
            .Build();
    ...
    public void Content_is_valid_json()
    {
        ...
        // Act
        var message = new FcmMessageBuilder()
            .AddContent(new NotificationContent(IdentityAddress.Parse("did:web:prod.enmesh.eu:dids:id1KJnD8ipfckRQ1ivAhNVLtypmcVM5vPX4j"), DevicePushIdentifier.Parse("DPIaaaaaaaaaaaaaaaaa"), new { SomeProperty = "someValue" }))
            .Build();
        var contentJson = FormatJson(message.Data["content"]);

        // Assert
        contentJson.Should().Be(FormatJson(@"{
          'accRef': 'did:web:prod.enmesh.eu:dids:id1KJnD8ipfckRQ1ivAhNVLtypmcVM5vPX4j',
