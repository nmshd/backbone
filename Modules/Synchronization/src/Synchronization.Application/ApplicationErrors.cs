...
public static class ApplicationErrors
{
    public static class Datawallet
    {
        ...
        {
            return new ApplicationError("error.platform.validation.datawallet.cannotPushDatawalletModificationsDuringActiveSyncRun",
                "There is an active sync run. In order to avoid conflicts it is not allowed to push new modifications while a sync run is active. Try again in a few seconds.");
        }

        public static ApplicationError InsufficientSupportedDatawalletVersion()
        {
            return new ApplicationError("error.platform.validation.datawallet.insufficientSupportedDatawalletVersion",
                "The supported datawallet version sent with the request is lower than the actual version of the datawallet.");
        }
    }
    ...
    public static class SyncRuns
    {
        ...
        {
            return new ApplicationError("error.platform.validation.syncRun.cannotFinalizeSyncRunStartedByAnotherDevice",
                "This sync was started by another device. You can only finalize a sync run with the device it was started from.");
        }

        public static ApplicationError CannotRefreshExpirationTimeOfSyncRunStartedByAnotherDevice()
        {
            return new ApplicationError("error.platform.validation.syncRun.cannotRefreshExpirationTimeOfSyncRunStartedByAnotherDevice",
                "This sync was started by another device. You can only extend the expiration time of a sync run with the device it was started from.");
        }

        public static ApplicationError CannotReadExternalEventsOfSyncRunStartedByAnotherDevice()
        {
            return new ApplicationError("error.platform.validation.syncRun.cannotReadExternalEventsOfASyncRunStartedByAnotherDevice",
                "This sync was started by another device. Only that device can read the corresponding external events.");
        }
        ...
        public static ApplicationError CannotStartSyncRunWhenAnotherSyncRunIsRunning(SyncRunId? idOfRunningSyncRun = null)
        {
            ...
            return new ApplicationError("error.platform.validation.syncRun.cannotStartSyncRunWhenAnotherSyncRunIsRunning",
                $"Another sync run is currently active{idString}. There can only be one active sync run at a time. Try again in a few seconds.");
        }
        ...
        {
            return new ApplicationError("error.platform.validation.syncRun.cannotStartSyncRunWhileIdentityIsToBeDeleted",
                "Cannot start sync run because the identity is in status 'ToBeDeleted'.");
        }
    }
}
