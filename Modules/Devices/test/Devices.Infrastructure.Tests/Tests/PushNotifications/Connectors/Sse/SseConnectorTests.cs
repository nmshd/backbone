...
public class SseConnectorTests : AbstractTestsBase
{
    ...
    public async Task Returns_a_success_if_a_registration_is_passed()
    {
        ...
        // Act
        var results = await sseConnector.Send([CreatePnsRegistration()], CreateRandomIdentityAddress(), new TestPushNotification());
    ...
    public async Task Uses_the_SseServerClient_to_send_the_event()
    {
        ...
        // Act
        await sseConnector.Send([CreatePnsRegistration()], recipient, new TestPushNotification());
    ...
    public async Task Returns_a_failure_with_reason_InvalidHandle_if_SseServerClient_throws_a_SseClientNotRegisteredException()
    {
        ...
        // Act
        var results = await sseConnector.Send([CreatePnsRegistration()], recipient, new TestPushNotification());
    ...
    {
        sseServerClient ??= A.Dummy<SseServerClient>();
