...
Scenario: Creating a Token as an authenticated user
	...
	Then the response status code for the Create Token request is 201 (Created)
...
Scenario: Creating a Token as an anonymous user
	...
	When a POST request is sent to the Tokens endpoint with
		| Key     | Value                                         |
		| Content | {"content": "QQ==","expiresAt": "<tomorrow>"} |
	Then the response status code for the Create Token request is 401 (Unauthorized)
...
Scenario: Creating a Token without the "Content" request content property
	...
	Then the response status code for the Create Token request is 400 (Bad Request)
...
Scenario: Creating a Token with an unsupported Content-Type
	...
	Then the response status code for the Create Token request is 415 (Unsupported Media Type)
