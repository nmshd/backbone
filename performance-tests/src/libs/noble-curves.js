...
var nobleCurves = (() => {
    ...
    var SHA256_IV = /* @__PURE__ */ new Uint32Array([1779033703, 3144134277, 1013904242, 2773480762, 1359893119, 2600822924, 528734635, 1541459225]);
    ...
    function tonelliShanks(P3) {
        ...
        if (S === 1) {
            ...
            return function tonelliFast(Fp8, n) {
                ...
                if (!Fp8.eql(Fp8.sqr(root), n)) throw new Error("Cannot find square root");
                return root;
            };
        }
        const Q1div2 = (Q + _1n2) / _2n2;
        return function tonelliSlow(Fp8, n) {
            if (Fp8.pow(n, legendreC) === Fp8.neg(Fp8.ONE)) throw new Error("Cannot find square root");
            let r = S;
    ...
    function weierstrassPoints(opts) {
        ...
        if (!Fp8.eql(Fp8.sqr(CURVE2.Gy), weierstrassEquation(CURVE2.Gx))) throw new Error("bad generator point: equation left != right");
        ...
        class Point2 {
            ...
            static fromAffine(p) {
                const { x, y } = p || {};
                if (!p || !Fp8.isValid(x) || !Fp8.isValid(y)) throw new Error("invalid affine point");
            ...
            assertValidity() {
                ...
                if (!Fp8.isValid(x) || !Fp8.isValid(y)) throw new Error("bad point: x or y not FE");
                const left = Fp8.sqr(y);
                const right = weierstrassEquation(x);
                if (!Fp8.eql(left, right)) throw new Error("bad point: equation left != right");
    ...
    function weierstrass(curveDef) {
        ...
        function int2octets(num) {
            ...
            if (!(_0n4 <= num && num < ORDER_MASK)) throw new Error(`bigint expected < 2^${CURVE2.nBitLength}`);
    ...
    function mapToCurveSimpleSWU(Fp8, opts) {
        validateField(Fp8);
        if (!Fp8.isValid(opts.A) || !Fp8.isValid(opts.B) || !Fp8.isValid(opts.Z)) throw new Error("mapToCurveSimpleSWU: invalid opts");
    ...
    function twistedEdwards(curveDef) {
        ...
        class Point2 {
            ...
            static fromHex(hex, zip215 = false) {
                ...
                if (!zip215 && x === _0n6 && isLastByteOdd) throw new Error("Point.fromHex: x=0 and x_0=1");
    ...
    function montgomery(curveDef) {
        ...
        function scalarMult(scalar, u) {
            ...
            if (pu === _0n7) throw new Error("Invalid private or public key received");
    ...
    function bls(CURVE2) {
        ...
        function pairing(Q, P3, withFinalExponent = true) {
            if (Q.equals(G1.ProjectivePoint.ZERO) || P3.equals(G2.ProjectivePoint.ZERO)) throw new Error("pairing is not available for ZERO point");
            Q.assertValidity();
            P3.assertValidity();
            const Qa = Q.toAffine();
            ...
            try {
                ...
                for (const message of new Set(nMessages)) {
                    const groupPublicKey = nMessages.reduce(
                        (groupPublicKey2, subMessage, i) => (subMessage === message ? groupPublicKey2.add(nPublicKeys[i]) : groupPublicKey2),
    ...
    var Fp2Multiply = ({ c0, c1 }, rhs) => {
        if (typeof rhs === "bigint") return { c0: Fp7.mul(c0, rhs), c1: Fp7.mul(c1, rhs) };
    ...
    var Fp22 = {
        ...
        // Bytes util
        fromBytes(b) {
            if (b.length !== Fp22.BYTES) throw new Error(`fromBytes wrong length=${b.length}`);
    ...
    var FP2_FROBENIUS_COEFFICIENTS = [BigInt("0x1"), BigInt("0x1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaaa")].map((item) => Fp7.create(item));
    ...
    var Fp62 = {
        ...
        // Bytes utils
        fromBytes: (b) => {
            if (b.length !== Fp62.BYTES) throw new Error(`fromBytes wrong length=${b.length}`);
            return {
    ...
    var Fp12Multiply = ({ c0, c1 }, rhs) => {
        if (typeof rhs === "bigint") return { c0: Fp62.mul(c0, rhs), c1: Fp62.mul(c1, rhs) };
        let { c0: r0, c1: r1 } = rhs;
    ...
    function setMask(bytes2, mask) {
        if (bytes2[0] & 224) throw new Error("setMask: non-empty mask");
        if (mask.compressed) bytes2[0] |= 128;
        if (mask.infinity) bytes2[0] |= 64;
        if (mask.sort) bytes2[0] |= 32;
        return bytes2;
    }
    ...
    function signatureG2ToRawBytes(point) {
        point.assertValidity();
        const len = Fp7.BYTES;
        if (point.equals(bls12_381.G2.ProjectivePoint.ZERO)) return concatBytes(COMPRESSED_ZERO, numberToBytesBE(_0n10, len));
        const { x, y } = point.toAffine();
        const { re: x0, im: x1 } = Fp22.reim(x);
    ...
    var bls12_381 = bls({
        ...
        G1: {
            ...
            fromBytes: (bytes2) => {
                ...
                if (value.length === 48 && compressed) {
                    ...
                    if (infinity) {
                        if (x !== _0n10 || y !== _0n10) throw new Error("G1: non-empty point at infinity");
        ...
        return Q;
            ...
            toBytes: (c, point, isCompressed) => {
                ...
                const { x, y } = point.toAffine();
                if (isCompressed) {
                    if (isZero) return concatBytes(COMPRESSED_ZERO, numberToBytesBE(_0n10, len));
