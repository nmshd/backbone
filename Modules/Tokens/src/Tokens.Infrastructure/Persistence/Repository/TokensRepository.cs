...
public class TokensRepository : ITokensRepository
{
    ...
    public async Task<DbPaginationResult<Token>> ListWithContent(PaginationFilter paginationFilter, Expression<Func<Token, bool>> filter, CancellationToken cancellationToken, bool track = false)
    {
        var query = (track ? _tokensDbSet : _readonlyTokensDbSet)
            .IncludeAll(_dbContext)
            .Where(filter);
