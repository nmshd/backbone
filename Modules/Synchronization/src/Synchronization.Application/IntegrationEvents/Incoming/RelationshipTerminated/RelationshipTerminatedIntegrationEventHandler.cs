...
public class RelationshipTerminatedIntegrationEventHandler : IIntegrationEventHandler<RelationshipTerminatedIntegrationEvent>
{
    ...
    public RelationshipTerminatedIntegrationEventHandler(ISynchronizationDbContext dbContext, IEventBus eventBus, ILogger<MessageCreatedIntegrationEventHandler> logger)
    {
    ...
    public async Task Handle(RelationshipTerminatedIntegrationEvent integrationEvent)
    {
        await CreateExternalEvents(integrationEvent);
    }

    private async Task CreateExternalEvents(RelationshipTerminatedIntegrationEvent @event)
    {
...
#pragma warning restore IDE0037
        try
        {
            var externalEvent = await _dbContext.CreateExternalEvent(IdentityAddress.Parse(@event.Partner), ExternalEventType.RelationshipTerminated, payload);
