...
Scenario: Requesting a Challenge as an authenticated user
	...
	And a Challenge c
	When a GET request is sent to the Challenges/{id} endpoint with Id "c.Id"
...
Scenario: Requesting a nonexistent Challenge as an authenticated user
	Given the user is authenticated
	When a GET request is sent to the Challenges/{id} endpoint with Id "CHLthisisnonexisting"
	Then the response status code is 404

Scenario: Requesting a Challenge with an invalid id as an authenticated user
	Given the user is authenticated
	When a GET request is sent to the Challenges/{id} endpoint with Id "a123"
...
Scenario: Requesting a Challenge with an unsupported Accept Header as an authenticated user
	...
	When a GET request is sent to the Challenges/{id} endpoint with Id "a valid Id"
